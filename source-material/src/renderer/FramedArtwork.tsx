...
function ProfileFrame({openingWidthMm,openingHeightMm,materialMode,moulding,profile}:{openingWidthMm:number;openingHeightMm:number;materialMode:Props['materialMode'];moulding:Moulding;profile:ProfilePoint[]}) {
  ...
  if(moulding.sku==='POL-4875') return <mesh geometry={geometry} castShadow receiveShadow><meshStandardMaterial map={map} color="#7b7780" roughness={.5} metalness={.22} envMapIntensity={.16}/></mesh>;
  return <mesh geometry={geometry} castShadow receiveShadow><meshPhysicalMaterial color={moulding.baseColor} roughness={moulding.roughness} metalness={.025} clearcoat={.07} clearcoatRoughness={.72} envMapIntensity={.22}/></mesh>;
}
...
function Scene(p:Props){
 ...
 const wall=p.displayMode==='Wall', room=roomPresets[p.wallPreset]||roomPresets['Warm Living Room'], cam=wall?[0,.1,9.2]:p.view==='Front'?[0,0,3.4]:p.view==='Detail'?[ow*.28,oh*.28,1.25]:[ow*.35,oh*.22,3.05];
 ...
 </group>{!wall&&<ContactShadows position={[0,-1.1,-.2]} opacity={.45} scale={6} blur={2.5}/>}<OrbitControls key={`controls-${p.displayMode}`} target={[0,wall ? .1 : 0,0]} enableRotate={!wall} enablePan={false} minDistance={wall?7.2:1.05} maxDistance={wall?11:4.2} minPolarAngle={Math.PI*.36} maxPolarAngle={Math.PI*.64}/><PerspectiveCamera key={`camera-${p.displayMode}-${p.view}`} makeDefault position={cam as [number,number,number]} fov={wall?36:p.view==='Detail'?27:33}/></>;
}
