...
import { roomAdminById, roomAdminRooms, roomCalibrationKey } from '../renderer/roomAdminRegistry';
...
type Draft={
 ...
 manualCalibration:{quadNormalised:number[][];scaleReference?:{label:string;lengthMm:number;pointsNormalised:number[][]}};
...
export default function RoomCalibrator(){
 ...
 const [roomId,setRoomId]=useState(roomAdminById(initialRoomId).id);
 const [draft,setDraft]=useState<Draft|null>(null);
 const [points,setPoints]=useState<Point[]>([]);
 const [scalePoints,setScalePoints]=useState<Point[]>([]);
 const [referenceMm,setReferenceMm]=useState(2400);
 const [active,setActive]=useState<{kind:'plane'|'scale';index:number}|null>(null);
 const [checks,setChecks]=useState([false,false,false,false]);
 const [saved,setSaved]=useState(false);
 const svgRef=useRef<SVGSVGElement>(null);
 const selectedRoom=roomAdminById(roomId);
 ...
 useEffect(()=>{
  ...
  fetch(selectedRoom.draft).then(response=>response.json()).then((record:Draft)=>{
  setDraft(record);
 ...
 if(active===null)return;
  ...
  else setScalePoints(current=>current.map((point,index)=>index===active.index?bounded:point));
 };
 const approvedRecord=()=>draft&&({
  ...
  manualCalibration:{...draft.manualCalibration,quadNormalised:points.map(point=>[Number(point.x.toFixed(6)),Number(point.y.toFixed(6))]),scaleReference:{label:'Target-wall floor to ceiling',lengthMm:referenceMm,pointsNormalised:scalePoints.map(point=>[Number(point.x.toFixed(6)),Number(point.y.toFixed(6))])},reviewedAt:new Date().toISOString(),reviewer:'local-review'},
  renderer:{
 ...
 const reset=()=>{if(!draft)return;const scale=draft.manualCalibration.scaleReference||defaultScaleReference;setPoints(draft.manualCalibration.quadNormalised.map(([x,y])=>({x,y})));setScalePoints(scale.pointsNormalised.map(([x,y])=>({x,y})));setReferenceMm(scale.lengthMm);setChecks([false,false,false,false]);setSaved(false);localStorage.removeItem(calibrationKey);};

 if(!draft||!project)return <main className="cal-loading">Loading reconstruction evidence…</main>;
 const insets=[.1,.16];
 return <main className="cal-app">
  <section className="cal-workspace">
   ...
   <div className="cal-image-wrap">
    ...
    <svg ref={svgRef} viewBox="0 0 1 1" preserveAspectRatio="none" onPointerMove={move} onPointerUp={()=>setActive(null)} onPointerCancel={()=>setActive(null)}>
     ...
     <polygon points={[project(insets[0],insets[0]),project(1-insets[0],insets[0]),project(1-insets[0],1-insets[0]),project(insets[0],1-insets[0])].map(p=>`${p.x},${p.y}`).join(' ')} className="frame-proxy" vectorEffect="non-scaling-stroke"/>
     <polygon points={[project(insets[1],insets[1]),project(1-insets[1],insets[1]),project(1-insets[1],1-insets[1]),project(insets[1],1-insets[1])].map(p=>`${p.x},${p.y}`).join(' ')} className="mount-proxy" vectorEffect="non-scaling-stroke"/>
   ...
   <div className="cal-caption"><b>Gold sets perspective; cyan sets physical scale.</b> The dark proxy checks wall alignment only and is not a size guide.</div>
  </section>
  <aside className="cal-panel">
   ...
   <label className="cal-room-picker"><span>Room draft</span><select value={roomId} onChange={event=>setRoomId(event.target.value)}>{roomAdminRooms.map(room=><option value={room.id} key={room.id}>{room.name}</option>)}</select></label>
