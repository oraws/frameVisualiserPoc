...
import { roomAdminRooms, roomCalibrationKey, roomPreviewKey, roomVisibilityKey, type RoomAdminDefinition } from '../renderer/roomAdminRegistry';
...
type Draft={
 ...
 renderer?:{cameraFovDegrees?:number;wallYawDegrees?:number;wallPitchDegrees?:number;perspectiveMode?:string;lighting?:LightingControls};
};
type LightingControls={azimuth:number;elevation:number;keyStrength:number;ambientStrength:number;temperature:number;shadowOpacity:number;shadowSoftness:number;shadowGapMm:number};

const defaultScaleReference={label:'Target-wall floor to ceiling',lengthMm:2400,pointsNormalised:[[.12,.18],[.12,.72]]};
const defaultLighting:LightingControls={azimuth:-.55,elevation:.65,keyStrength:1,ambientStrength:.72,temperature:4400,shadowOpacity:.34,shadowSoftness:10,shadowGapMm:10};
...
function homographyMatrix(points:Point[]){
 const [p0,p1,p2,p3]=points;
 const dx1=p1.x-p2.x,dx2=p3.x-p2.x,dy1=p1.y-p2.y,dy2=p3.y-p2.y;
 const sx=p0.x-p1.x+p2.x-p3.x,sy=p0.y-p1.y+p2.y-p3.y,denominator=dx1*dy2-dx2*dy1;
...
function inverse3(m:number[]){
 const [a,b,c,d,e,f,g,h,i]=m,A=e*i-f*h,B=c*h-b*i,C=b*f-c*e,D=f*g-d*i,E=a*i-c*g,F=c*d-a*f,G=d*h-e*g,H=b*g-a*h,I=a*e-b*d,det=a*A+b*D+c*G;
 if(Math.abs(det)<1e-10)return null;
 return[A/det,B/det,C/det,D/det,E/det,F/det,G/det,H/det,I/det];
}
function matrixPoint(m:number[],point:Point){const q=m[6]*point.x+m[7]*point.y+m[8];return{x:(m[0]*point.x+m[1]*point.y+m[2])/q,y:(m[3]*point.x+m[4]*point.y+m[5])/q};}
function temperatureCss(kelvin:number){
 const t=Math.max(10,Math.min(400,kelvin/100));let r:number,g:number,b:number;
 if(t<=66){r=255;g=99.4708025861*Math.log(t)-161.1195681661;b=t<=19?0:138.5177312231*Math.log(t-10)-305.0447927307;}else{r=329.698727446*Math.pow(t-60,-.1332047592);g=288.1221695283*Math.pow(t-60,-.0755148492);b=255;}
...
export default function RoomCalibrator(){
 ...
 const [roomLighting,setRoomLighting]=useState<LightingControls>(defaultLighting);
 ...
 useEffect(()=>{
  ...
  setDraft(null);setSaved(false);setChecks([false,false,false]);
  ...
  setDraft(record);
  const stored=localStorage.getItem(previewKey)||localStorage.getItem(calibrationKey);
  if(stored){
   try{const parsed=JSON.parse(stored);if((parsed.approved||parsed.preview)&&parsed.manualCalibration?.quadNormalised){
   setPoints(parsed.manualCalibration.quadNormalised.map(([x,y]:number[])=>({x,y})));
    ...
    setCameraFov(parsed.renderer?.cameraFovDegrees||record.geometry.camera.horizontalFovDegrees);setRoomLighting({...defaultLighting,...parsed.renderer?.lighting});
  ...
  setCameraFov(record.renderer?.cameraFovDegrees||record.geometry.camera.horizontalFovDegrees);setRoomLighting({...defaultLighting,...record.renderer?.lighting});
  setReferenceMm(scale.lengthMm);
 });},[roomId,adminRooms.length]);

 const project=useMemo(()=>points.length===4?homography(points):null,[points]);
 const previewGeometry=useMemo(()=>{
  if(points.length!==4||scalePoints.length!==2||referenceMm<=0)return null;
  const inverse=inverse3(homographyMatrix(points));if(!inverse)return null;
  const a=matrixPoint(inverse,scalePoints[0]),b=matrixPoint(inverse,scalePoints[1]),sourcePerMm=Math.hypot(b.x-a.x,b.y-a.y)/referenceMm;
  const outerWidth=994*sourcePerMm,outerHeight=794*sourcePerMm,mountWidth=800*sourcePerMm,mountHeight=600*sourcePerMm,artWidth=700*sourcePerMm,artHeight=500*sourcePerMm;
  const corners=(width:number,height:number,dx=0,dy=0)=>[[-1,-1],[1,-1],[1,1],[-1,1]].map(([sx,sy])=>project!((frameCentre.x+dx)+sx*width/2,(frameCentre.y+dy)+sy*height/2));
  const shadowScale=roomLighting.shadowGapMm/Math.max(referenceMm,1)*4,shadowDx=-roomLighting.azimuth*shadowScale,shadowDy=Math.max(.15,roomLighting.elevation)*shadowScale;
  return{outer:corners(outerWidth,outerHeight),mount:corners(mountWidth,mountHeight),art:corners(artWidth,artHeight),shadow:corners(outerWidth,outerHeight,shadowDx,shadowDy)};
 ...
 const calibrationRecord=(approved:boolean)=>draft&&({
  ...draft,
 ...
 const previewInViewer=()=>{const record=calibrationRecord(false);if(!record)return;localStorage.setItem(previewKey,JSON.stringify(record));window.location.assign(`/?mode=wall&room=${roomId}&preview=1`);};
 const reset=()=>{if(!draft)return;const scale=draft.manualCalibration.scaleReference||defaultScaleReference;const centre=draft.manualCalibration.frameCentreNormalised||[.5,.5];setPoints(draft.manualCalibration.quadNormalised.map(([x,y])=>({x,y})));setFrameCentre({x:centre[0],y:centre[1]});setScalePoints(scale.pointsNormalised.map(([x,y])=>({x,y})));setReferenceMm(scale.lengthMm);setCameraFov(draft.geometry.camera.horizontalFovDegrees);setRoomLighting({...defaultLighting,...draft.renderer?.lighting});setChecks([false,false,false]);setSaved(false);localStorage.removeItem(calibrationKey);localStorage.removeItem(previewKey);};
 ...
 const updateLighting=(key:keyof LightingControls,value:number)=>{setRoomLighting(current=>({...current,[key]:value}));setSaved(false)};
 ...
 return <main className="cal-app">
  <section className="cal-workspace">
   ...
   <div className="cal-image-wrap">
    ...
    <svg ref={svgRef} viewBox="0 0 1 1" preserveAspectRatio="none" onPointerMove={move} onPointerUp={()=>setActive(null)} onPointerCancel={()=>setActive(null)}>
     <defs><linearGradient id="frame-light-preview" x1={`${50-roomLighting.azimuth*45}%`} y1={`${85-roomLighting.elevation*70}%`} x2={`${50+roomLighting.azimuth*45}%`} y2={`${15+roomLighting.elevation*30}%`}><stop offset="0" stopColor="#090909"/><stop offset=".58" stopColor="#242322"/><stop offset="1" stopColor={temperatureCss(roomLighting.temperature)} stopOpacity={Math.min(.72,.18+roomLighting.keyStrength*.2)}/></linearGradient><filter id="frame-shadow-preview" x="-40%" y="-40%" width="180%" height="180%"><feGaussianBlur stdDeviation={.0015+roomLighting.shadowSoftness*.00045}/></filter></defs>
     ...
     {previewGeometry&&<><polygon points={previewGeometry.shadow.map(p=>`${p.x},${p.y}`).join(' ')} className="frame-shadow-proxy" style={{opacity:roomLighting.shadowOpacity}} filter="url(#frame-shadow-preview)"/><polygon points={previewGeometry.outer.map(p=>`${p.x},${p.y}`).join(' ')} className="frame-proxy" fill="url(#frame-light-preview)" vectorEffect="non-scaling-stroke"/><polygon points={previewGeometry.mount.map(p=>`${p.x},${p.y}`).join(' ')} className="mount-proxy" vectorEffect="non-scaling-stroke"/><polygon points={previewGeometry.art.map(p=>`${p.x},${p.y}`).join(' ')} className="art-proxy" vectorEffect="non-scaling-stroke"/></>}
   ...
   <div className="cal-caption"><b>Gold: perspective · cyan: physical scale · pink: frame centre.</b> The live proxy is a 700 × 500 mm artwork in the current Verona frame.</div>
  </section>
  <aside className="cal-panel">
   ...
   <dl>
    ...
    <div><dt>Lighting</dt><dd>Admin directional estimate</dd></div>
   </dl>
   ...
   <section className="cal-lighting"><h2>Perspective</h2><p>The gold quadrilateral is now the single source of truth. The viewer reproduces that plane instead of applying a separate guessed wall angle.</p><AdminRange label="Camera field of view" value={cameraFov} min={28} max={80} step={1} display={`${cameraFov}°`} onChange={value=>{setCameraFov(value);setSaved(false)}}/>
   <h2>Room lighting</h2><p>The frame highlight and wall shadow on the photograph update immediately. Use the full viewer for the moulding’s detailed 3D response.</p><AdminRange label="Light left / right" value={roomLighting.azimuth} min={-1} max={1} step={.05} display={roomLighting.azimuth<-.08?'Left':roomLighting.azimuth>.08?'Right':'Centre'} onChange={value=>updateLighting('azimuth',value)}/><AdminRange label="Light height" value={roomLighting.elevation} min={0} max={1} step={.05} display={`${Math.round(roomLighting.elevation*100)}%`} onChange={value=>updateLighting('elevation',value)}/><AdminRange label="Key strength" value={roomLighting.keyStrength} min={.2} max={2.5} step={.05} display={`${Math.round(roomLighting.keyStrength*100)}%`} onChange={value=>updateLighting('keyStrength',value)}/><AdminRange label="Ambient fill" value={roomLighting.ambientStrength} min={.1} max={1.5} step={.05} display={`${Math.round(roomLighting.ambientStrength*100)}%`} onChange={value=>updateLighting('ambientStrength',value)}/><AdminRange label="Colour temperature" value={roomLighting.temperature} min={2700} max={7500} step={100} display={`${roomLighting.temperature}K`} onChange={value=>updateLighting('temperature',value)}/><AdminRange label="Shadow density" value={roomLighting.shadowOpacity} min={.05} max={.8} step={.025} display={`${Math.round(roomLighting.shadowOpacity*100)}%`} onChange={value=>updateLighting('shadowOpacity',value)}/><AdminRange label="Shadow softness" value={roomLighting.shadowSoftness} min={1} max={20} step={1} display={`${roomLighting.shadowSoftness}`} onChange={value=>updateLighting('shadowSoftness',value)}/><AdminRange label="Frame off wall" value={roomLighting.shadowGapMm} min={2} max={35} step={1} display={`${roomLighting.shadowGapMm} mm`} onChange={value=>updateLighting('shadowGapMm',value)}/><button type="button" className="cal-preview" onClick={previewInViewer}>Preview detailed frame in visualiser ↗</button></section>
...
function AdminRange({label,value,min,max,step,display,onChange}:{label:string;value:number;min:number;max:number;step:number;display:string;onChange:(value:number)=>void}){return <label className="cal-range"><span>{label}<b>{display}</b></span><input type="range" aria-label={label} min={min} max={max} step={step} value={value} onChange={event=>onChange(Number(event.target.value))}/></label>}
